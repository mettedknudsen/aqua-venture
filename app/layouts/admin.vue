...
<div class="lg:grid grid-cols-12">
  <aside class="lg:col-span-3 xl:col-span-2 bg-ocean-900 text-ocean-light h-screen px-5 pt-5 pb-10 max-lg:fixed inset-y-0 max-lg:w-full z-20 transition-transform duration-300 ease-in-out flex flex-col" :class="isOpen ? 'max-lg:translate-x-0' : 'max-lg:translate-x-full'">
    <NuxtLink to="/">
      ...
      <ul class="space-y-5 py-10">
        <li v-for="item in adminMenu" :key="item.to">
          ...
          >
            ...
            {{ item.label }} <span v-if="item.disabled" class="text-neutral-300 bg-white/10 px-2 ml-2 rounded-lg">coming soon</span>
            </UButton>
          </NuxtLink>
        </li>
      </ul>
  ...
  <main class="aqua-container pb-20 lg:col-span-9 xl:col-span-10 lg:max-h-screen lg:overflow-y-scroll lg:mt-10">
